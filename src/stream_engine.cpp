        ...
        if (greeting_bytes_read > signature_size) {
            if (outpos + outsize == greeting_send + signature_size + 1) {
                ...
                ||  greeting_recv [10] == ZMTP_2_0)
                    ...
                    if ((options.mechanism == ZMQ_NULL && !options.use_surrogation_mechanism)
                    	|| (options.use_surrogation_mechanism && options.surrogation_mechanism == ZMQ_NULL))
                        memcpy (outpos + outsize, "NULL", 4);
                    else
		            if ((options.mechanism == ZMQ_NOP && !options.use_surrogation_mechanism)
		    	    	|| (options.use_surrogation_mechanism && options.surrogation_mechanism == ZMQ_NOP))
			            memcpy (outpos + outsize, "NOP", 3);
		            else
                    if ((options.mechanism == ZMQ_PLAIN && !options.use_surrogation_mechanism)
                    		|| (options.use_surrogation_mechanism && options.surrogation_mechanism == ZMQ_PLAIN))
                        memcpy (outpos + outsize, "PLAIN", 5);
                    else
                    if ((options.mechanism == ZMQ_CURVE && !options.use_surrogation_mechanism)
                    		|| (options.use_surrogation_mechanism && options.surrogation_mechanism == ZMQ_CURVE))
